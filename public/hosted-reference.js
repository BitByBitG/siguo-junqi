// 网站托管示例：在 BOT 工作台粘贴运行，无 Node.js 权限。
...
function validateSetup(pieces, seat) {
...
function pieceAt(pieces, position) {
...
function validateMove(room, seat, from, to) {
...
function revealFlagWhenCommanderDies(room, piece) {
...
function observeBotBattle(room, attacker, defender, outcome) {
  if (!defender) return;
...
function engineerTurnPenalty(view, move) {
...
function samplePosition(view, domains, rng = Math.random, deadline = Infinity) {
  const records = view.records || view.pieces;
...
function strongMoves(state, seat = state.turn) {
...
function simulateStrongMove(state, move) {
...
function searchPosition(state, root, depth, alpha, beta, deadline, ply = 0, exchanges = 2) {
...
function rootHeuristic(view, move, history) {
  ...
  score -= history.filter(m => m.from === move.from && m.to === move.to).length * 32;
  score -= history.slice(-6).filter(m => m.from === move.to && m.to === move.from).length * 20;
  const progress = targets.length ? Math.min(...targets.map(n => dist(from, n))) - Math.min(...targets.map(n => dist(to, n))) : 0;
  score += progress * 1.5;
  if (!d && progress <= 0) score -= 8;
...
function chooseStrongMove(view, history = [], { budgetMs = 4700, rng = Math.random, onProgress = () => {} } = {}) {
  const started = Date.now(), deadline = started + Math.max(1, budgetMs);
  const legal = legalBotMoves(view);
  ...
  const candidates = ordered.slice(0, 24);
...
function scoreStrongSetup(army, seat) {
...
function createStrongArmy(seat, budgetMs = 120, rng = Math.random) {
...
function act(state) {
  ...
  const move = chooseStrongMove(state, history, { budgetMs: state.thinkTimeMs }).move || state.legalMoves[0];
