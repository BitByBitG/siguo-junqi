...
credentialForm.addEventListener("submit", async event => {
  ...
  try {
    const changing = ['/password.html','/settings.html'].includes(location.pathname);
    ...
    if (changing) { localStorage.removeItem("junqi-auth"); localStorage.removeItem("junqi-session"); window.dispatchEvent(new Event('junqi-profile-refresh')); }
